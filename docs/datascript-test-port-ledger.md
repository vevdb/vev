...
| `parser*.cljc` | 19 | partial | query text subset parses relation/collection/pull find, scalar/collection/tuple/relation `:in`, optional `:where`, data, predicate, built-in function, missing?, not, or, ordinary rule calls/definitions, and transaction text parses common vector/map tx-data; full EDN source inputs, pull options, source-qualified rules, and validation remain |
