...
| `pull_api.cljc` | 17 | subset | named `:xform` transforms for `vector`/`name` covered; arbitrary host xform functions, visitor options, and exact collection/scalar render shape remain |
...
| `upsert.cljc` | 6 | subset | unique cardinality-many, multi-identity convergence, unique-value no-upsert, vector tx tempid ordering, redefining tempids, current-tx and explicit-id identity conflicts, forward string tempid refs, and non-upsert of new ref tempids covered; exact messages remain |
