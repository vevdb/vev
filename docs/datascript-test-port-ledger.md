...
| `parser*.cljc` | 19 | partial | query text subset parses vector and map query forms, scalar/collection/tuple/pull/aggregate find specs, pull pattern variables including bare pattern names, flat and nested pull option forms including named `:xform`, option-wrapped pull map keys, `:with`, return-map markers for keyed text helpers, scalar/collection/tuple/relation/relation-source/named-DB-source `:in`, nested input/result destructuring, preserves DataScript source-order binding for mixed input kinds and top-level `:where` execution, map relation inputs through `query-input-map-relation`, optional `:where`, data, predicate, built-in function, missing?, not/not-join, or/or-join, ordinary/source-qualified rule calls and rule definitions; Kvist literal query macros now preserve top-level `:where` source order too; query text now exposes parse, input arity, unknown source, unknown op, and insufficient binding errors through `Result-Set`; direct pull text APIs and transaction text common vector/map tx-data are covered; exact validation remains |
