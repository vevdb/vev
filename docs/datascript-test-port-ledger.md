...
| `query_fns.cljc` | 7 | subset | built-in predicates/functions including boolean/nil predicates, string contains/prefix/suffix helpers, integer arithmetic helpers, `name`, `subs`, regex `re-pattern`/`re-find`, nested result binding, result unification, lookup-ref inputs, and rule/function binding interaction covered; arbitrary host functions, `keyword`/`str`, regex capture-return functions, and exact errors remain |
...
| `parser*.cljc` | 19 | partial | query text subset parses scalar/collection/tuple/pull/aggregate find specs, flat and nested pull option forms, `:with`, return-map markers for keyed text helpers, scalar/collection/tuple/relation/relation-source/named-DB-source `:in`, nested input/result destructuring, preserves DataScript source-order binding for mixed input kinds, map relation inputs through `query-input-map-relation`, optional `:where`, data, predicate, built-in function, missing?, not/not-join, or/or-join, ordinary/source-qualified rule calls and rule definitions, and transaction text parses common vector/map tx-data; exact validation remains |
