...
| `index.cljc` | 5 | partial | main order, `find-datom` prefixes, seek/rseek/range, checked indexed-attribute errors, and sequence compare covered; public `datoms`/`seek-datoms`/`rseek-datoms`/`index-range` now use sorted-index binary bounds; finish exact public index surface |
