...
| `storage.clj` | 5 | partial | snapshot-file, SQLite datom-row durable reopen/query paths, and native wrapper-level append-on-transaction SQLite connection mode exist; exact DataScript storage API, host/ABI durable connection handles, and explicit tx metadata persistence remain |
