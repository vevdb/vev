...
| `query_rules.cljc` | 3 | partial | source args, recursion, repeated calls, ordered body execution for literal/text rules, inline map-query `:rules`, primary collection DB rule queries, and unknown/arity/param validation covered; host predicate inputs and semi-naive performance remain |
...
| `query_v3.cljc` | 2 | partial | input arity, non-collection source validation, and primary collection query execution covered through current query APIs; remaining query-v3 engine surface later |
