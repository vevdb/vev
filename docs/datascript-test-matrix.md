...
| `parser_query.cljc` | 1 | partial | EDN-reader-backed text query subset covers vector and map query forms, `:find`, `:with`, `:in`, named DB sources, optional `:where`, map relation inputs via helper, nested input/result destructuring, and execution through normal query inputs/sources; validation remains |
