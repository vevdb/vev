...
| `storage.clj` | 5 | partial | snapshot-file, SQLite datom-row durable reopen/query paths, native wrapper-level append-on-transaction SQLite connection mode, and SQLite tx metadata rows exist; exact DataScript storage API and host/ABI durable connection handles remain |
