...
- `partial` Add structured parser diagnostics and malformed-input suites.
  Portable prepared parser values now include stable `:error-code` categories across binding, `:in`, `:with`, rules, query, pull-pattern, and tx-data parser entry points, including `:edn-read` for reader-level malformed EDN across query, rules, pull, and tx-data values. Prepared query values also retain and render return-map marker plus typed key metadata for `:keys`, `:strs`, and `:syms`. Remaining work is exact malformed-input namespace coverage and DataScript/Clojure parser record or diagnostic text parity where that is worth exposing.
