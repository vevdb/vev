...
| `parser_*` | partial | Minimal query text API parses `[:find ... :where ...]` with simple data clauses and lowers to the same `Query` representation; a flat EDN reader now handles nested vectors/lists/maps as the foundation for full string APIs; full EDN lowering, predicates/functions, pull, rules, and validation remain |
