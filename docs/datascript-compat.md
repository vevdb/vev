...
| `parser_*` | partial | Text APIs are backed by the flat EDN reader. Query text parses `[:find ... :in ... :where ...]` with scalar, collection, tuple, pull, aggregate, and top-n aggregate find specs, keyed return-map markers, scalar/collection/tuple/relation/relation-source/named-DB-source inputs, data clauses including 2/3/4/5-wide relation-source rows, predicate clauses, built-in function clauses, `missing?`, `not`, `or`, ordinary/source-qualified rule calls and definitions, and no-`:where` input-only queries into the same `Query` representation. Transaction text parses common tx-data vectors and maps into normal `Tx-Data`; full EDN lowering for pull options and exact validation remains |
