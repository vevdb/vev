...
| `parser_*` | partial | Text APIs are backed by the flat EDN reader. Query text parses `[:find ... :where ...]` with data clauses, predicate clauses, built-in function clauses, `missing?`, `not`, `or`, and literal pull finds into the same `Query` representation. Transaction text parses common tx-data vectors and maps into normal `Tx-Data`; full EDN lowering for pull options, rules, query inputs, and exact validation remains |
...
2. Broaden EDN string interop for `:in`, query rules, pull options, and exact
   parser validation.
