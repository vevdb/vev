...
| `pull_parser` | partial | Kvist pull literals, query text pull finds, and direct `pull-text` APIs cover attrs, wildcard, nested map patterns, recursive map values, flat/nested `:default`/`:as`/`:limit` option vectors, and option-wrapped map keys; validation and xform remain |
