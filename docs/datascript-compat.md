...
| --- | --- | --- |
| `query` | partial | DataScript join subset ported including existence and multi-hop joins; inputs, no-where input-only queries, empty collection/relation inputs, plain int entity inputs, collection/tuple/relation bindings, cardinality-many joins, named relation source joins, multi-DB source joins, `_` input placeholders, and constant-substitution subset covered |
