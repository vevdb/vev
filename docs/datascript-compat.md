...
| `parser_*` | partial | Query text API is now backed by the flat EDN reader and parses `[:find ... :where ...]` with simple data clauses into the same `Query` representation; full EDN lowering for predicates/functions, pull, rules, tx data, and validation remains |
