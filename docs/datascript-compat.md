...
| `transact` | partial | Add/retract/map forms, nil item skipping, value-specific retract no-ops, incoming-ref cleanup on retractEntity, ref-typed numeric value resolution, current tx tempids, unused value-tempid rejection, intermediate-DB lookup/CAS resolution subset, cardinality-one/unique/default replacement, CAS one/many/nil/lookup-ref value subset, and CAS tempid rejection covered; tx fn call and exact errors incomplete |
