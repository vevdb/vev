...
| `transact` | partial | Add/retract/map forms, nil item skipping, value-specific retract no-ops, incoming-ref cleanup on retractEntity, ref-typed numeric value resolution, current tx tempids, forward/cyclic tempid ref values, unused value-tempid rejection, intermediate-DB lookup/CAS resolution subset, cardinality-one/unique/default replacement, CAS one/many/nil/lookup-ref value subset, and CAS tempid rejection covered; tx fn call and exact errors incomplete |
| `upsert` | partial | Unique-identity map/list tempid upsert, unique-value no-upsert enforcement, intermediate-db retry subset, string tempid refs including forward refs, unique-ref numeric/lookup-ref upsert, and conflicting unique-field subsets covered; exact conflict messages incomplete |
