...
| `lookup_refs` | partial | Lookup refs covered in query entity/value positions, scalar and collection query inputs, pull, tx entity/value/map-value/CAS entity/expected/value positions, missing-ref retract no-ops, and `datoms`/`seek-datoms` index prefixes; `index-range` lookup refs incomplete |
