...
| `query_fns` | partial | DataScript subset ported for predicates including `even?`, `odd?`, `zero?`, `pos?`, `neg?`, `true?`, `false?`, `nil?`, `some?`, `not`, `empty?`, string contains/prefix/suffix predicates, mixed-type predicate comparison, scalar/collection/tuple `ground`, `get-else`, `get-some`, lookup-ref inputs to query functions, `missing?`, literal vector/function args, tuple construction/destructuring, function-result unification with vector `first`/`second`, rule/function binding interaction, regex predicates via `re-pattern`/`re-find`, and built-in result clauses for `identity`, `inc`, `dec`, `name`, `subs`, `vector`, `tuple`, `untuple`, `hash-map`, `get`, string `count`, integer arithmetic, `quot`, `rem`, `mod`, `min`, `max`, and `compare`; arbitrary host/function clauses plus `keyword`, `str`, and regex capture-return functions remain |
