...
| `query_not` | partial | Single-source DataScript subset ported, including nested `not`, explicit named relation sources inside `not`, and direct source-prefixed `($src not ...)`; nested default-source inheritance and insufficient-binding errors incomplete |
| `query_or` | partial | Single-source DataScript subset ported, including scalar-input `or`, direct source-prefixed `($src or ...)`, and relation-source `or-join`; nested default-source inheritance and exact validation errors incomplete |
