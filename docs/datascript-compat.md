...
| `query_fns` | partial | DataScript subset ported for predicates including `even?`, `odd?`, `zero?`, `pos?`, `neg?`, `true?`, `false?`, `nil?`, `some?`, `not`, `empty?`, string contains/prefix/suffix predicates, mixed-type predicate comparison, scalar/collection/tuple/relation `ground` including EDN text parsing, `get-else` including EDN text parsing and nil-default rejection, `get-some` including vector/map EDN text parsing, lookup-ref inputs to query functions, `missing?`, literal vector/function args, tuple construction/destructuring, function-result unification with vector `first`/`second`, rule/function binding interaction, regex predicates and match-return clauses via `re-pattern`/`re-find`, and built-in result clauses for `identity`, `inc`, `dec`, `name`, `str`, `keyword`, `subs`, `vector`, `tuple`, `untuple`, `hash-map`, `get`, string `count`, integer arithmetic, `quot`, `rem`, `mod`, `min`, `max`, and `compare`; arbitrary host/function clauses and exact error behavior remain |
...
| `query_pull` | partial | Basic literal-pattern, `:in` variable-pattern including DataScript-style bare pattern names, lookup-ref input, multi-source pull, and pull-with-aggregates DataScript subsets ported; returning pattern values and exact tuple return shape incomplete |
| `query_return_map` | partial | DataScript subset ported for all-row `:keys`/`:strs`/`:syms` and tuple return maps |
| `query_rules` | partial | Literal and text rule input subsets started: rule branches plus fixpoint recursive/mutual/symmetric rules with data clauses, repeated rule calls, ordered body execution for data/predicate/function/ground/rule-call/not/or steps, predicates including `even?`, `ground`, built-in function clauses, false arguments, source-qualified relation rule calls, DataScript rule-vars required-argument syntax, duplicate rule var checks, branch arity checks, and validation for unknown rules, bad arity, and malformed rule params; host predicate inputs and semi-naive performance work incomplete |
| `parser_*` | partial | Text APIs are backed by the flat EDN reader. Query text parses `[:find ... :with ... :in ... :where ...]` with scalar, collection, tuple, pull, aggregate, and top-n aggregate find specs, keyed return-map markers including count/shape validation, scalar/collection/tuple/relation/relation-source/named-DB-source inputs, data clauses including 2/3/4/5-wide relation-source rows, predicate clauses, built-in function clauses, `ground`, `get-else`, `get-some`, `missing?`, top-level/rule-body `and`, `not`/`not-join`, `or`/`or-join`, ordinary/source-qualified rule calls and definitions including rule-vars required-argument vectors, no-`:where` input-only queries, bare-symbol pull pattern inputs, and ordered top-level `:where` execution/validation. Kvist literal query macros now lower top-level `:where` forms with the same ordered step metadata and rule-vars metadata. Pull text APIs parse EDN pull vectors directly, including named `:xform` options. Transaction text parses common tx-data vectors and maps into normal `Tx-Data`; exact validation remains |
...
| `index` | partial | `datoms`, `find-datom` including empty DB and prefix lookups, `seek-datoms`, `rseek-datoms`, upstream `index-range` bounds/order examples, and public AVET filtering covered; public access now uses binary lower/upper bounds over sorted in-memory indexes instead of whole-index scans; exact indexed-attribute error behavior incomplete |
