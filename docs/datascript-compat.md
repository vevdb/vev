...
| `parser_*` | partial | Query text API is backed by the flat EDN reader and parses `[:find ... :where ...]` with data clauses, predicate clauses, built-in function clauses, `missing?`, `not`, `or`, and literal pull finds into the same `Query` representation; full EDN lowering for pull options, rules, tx data, and validation remains |
...
| `transact` | partial | Add/retract/map forms, nil item skipping, value-specific retract no-ops including int-vs-float exactness, retract not-found/idempotency, mixed-type unique lookup/index compare, incoming-ref cleanup and adjacent tx-data reporting on retractEntity, ref-typed numeric value resolution, current tx tempids, forward/cyclic tempid ref values including generated-id map parents, map-valued datom compare/lookup subset, unused value-tempid rejection including empty cardinality-many map definitions, intermediate-DB lookup/CAS resolution subset, cardinality-one/unique/default replacement, CAS one/many/nil/lookup-ref value subset, tempids-outside-add rejection, and native transaction functions returning tx-data covered; ident-stored tx functions and exact errors incomplete |
