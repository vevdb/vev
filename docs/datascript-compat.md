...
| `query_fns` | partial | DataScript subset ported for predicates, mixed-type predicate comparison, `ground`, `get-else`, `get-some`, `missing?`, and built-in result clauses for `identity`, string `count`, and integer arithmetic; arbitrary host/function clauses missing |
