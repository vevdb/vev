...
| `parser_*` | partial | Query text API is backed by the flat EDN reader and parses `[:find ... :where ...]` with data clauses, predicate clauses, built-in function clauses, `missing?`, `not`, and `or` into the same `Query` representation; full EDN lowering for pull, rules, tx data, and validation remains |
