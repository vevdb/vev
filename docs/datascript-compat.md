...
| `parser_*` | partial | Query text API is backed by the flat EDN reader and parses `[:find ... :where ...]` with data clauses, predicate clauses, and built-in function clauses into the same `Query` representation; full EDN lowering for `not`/`or`, pull, rules, tx data, and validation remains |
