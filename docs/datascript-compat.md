...
| `parser_*` | partial | Text APIs are backed by the flat EDN reader. Query text parses `[:find ... :in ... :where ...]` with scalar, collection, tuple, and relation inputs, data clauses, predicate clauses, built-in function clauses, `missing?`, `not`, `or`, literal pull finds, ordinary rule calls/definitions, and no-`:where` input-only queries into the same `Query` representation. Transaction text parses common tx-data vectors and maps into normal `Tx-Data`; full EDN lowering for pull options, source-qualified rules/inputs, and exact validation remains |
...
2. Broaden EDN string interop for pull options, source-qualified rules/inputs, and exact
   parser validation.
